...
class HealthCheckInHelper(ZJULogin):
    ...
    def run(self):
        ...
        # 由于IP定位放到服务器上运行后会是服务器的IP定位， 因此这边需要在想要的IP位置上手动执行一次脚本
        ...
        if not json_data.get("location"):
            ...
            print("当前IP location为", location)
            if location.get("status") == 0:
                raise IPLocationError("定位失败")
            else:
                json_data['location'] = location
                with open(f_name, "w") as ff:
